...
pub mod staking_vault {
    ...
    pub fn stake(
        ...
        // Update vault state
        let staking_vault = &mut ctx.accounts.staking_vault;
        staking_vault.total_staked = staking_vault.total_staked.checked_add(amount).ok_or(ErrorCode::MathOverflow)?;
        ...
        user_stake_info.amount = user_stake_info.amount.checked_add(amount).ok_or(ErrorCode::MathOverflow)?;
    ...
    pub fn unstake(
        ...
        require!(amount > 0, ErrorCode::ZeroAmount);
        require!(user_stake_info.amount >= amount, ErrorCode::InsufficientStakedAmount);

        // Calculate pending rewards
        let pending_rewards = calculate_rewards(user_stake_info, staking_vault.current_apy_basis_points, now)?;

        // Update user stake info
        user_stake_info.amount = user_stake_info.amount.checked_sub(amount).ok_or(ErrorCode::MathOverflow)?;
        ...
        // Update vault state
        staking_vault.total_staked = staking_vault.total_staked.checked_sub(amount).ok_or(ErrorCode::MathOverflow)?;
        ...
        unstake_amount = unstake_amount.checked_add(pending_rewards).ok_or(ErrorCode::MathOverflow)?;
        ...
        msg!("Unstaked {} tokens with {} rewards", amount, pending_rewards);
        Ok(())
    }
...
// Calculate pending rewards
fn calculate_rewards(user_stake_info: &UserStakeInfo, apy_basis_points: u64, current_time: i64) -> Result<u64> {

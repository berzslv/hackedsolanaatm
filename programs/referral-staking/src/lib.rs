...
pub mod referral_staking {
    ...
    pub fn stake(ctx: Context<Stake>, amount: u64) -> Result<()> {
        ...
        user_info.staked_amount = user_info
            .staked_amount
            .checked_add(amount)
            .unwrap_or(user_info.staked_amount);
        ...
        global_state.total_staked = global_state
            .total_staked
            .checked_add(amount)
            .unwrap_or(global_state.total_staked);
        if is_new_staker {
            // This is a new staker
            global_state.stakers_count = global_state
                .stakers_count
                .checked_add(1)
                .unwrap_or(global_state.stakers_count);
        }
    ...
    pub fn update_referrer_rewards(
        ...
        let referral_reward =
            calculate_referral_reward(staking_amount, global_state.referral_reward_rate);
        ...
        referrer_info.rewards = referrer_info
            .rewards
            .checked_add(referral_reward)
            .unwrap_or(referrer_info.rewards);

        referrer_info.referral_count = referrer_info
            .referral_count
            .checked_add(1)
    ...
    pub fn unstake(ctx: Context<Unstake>, amount: u64) -> Result<()> {
        ...
        if time_staked < ctx.accounts.global_state.unlock_duration {
            ...
            msg!(
                "Time staked: {} seconds of {} required",
                time_staked,
                ctx.accounts.global_state.unlock_duration
            );
        } else {
        ...
        global_state.reward_pool = global_state
            .reward_pool
            .checked_add(penalty)
            .unwrap_or(global_state.reward_pool);
    ...
    pub fn compound_rewards(ctx: Context<CompoundRewards>) -> Result<()> {
        ...
        user_info.staked_amount = user_info
            .staked_amount
            .checked_add(rewards_to_compound)
            .unwrap_or(user_info.staked_amount);
        user_info.rewards = 0;
        user_info.last_stake_time = current_time;
    ...
    pub fn update_parameters(
        ...
        if let Some(referral_rate) = referral_reward_rate {
            ...
            msg!(
                "Updated referral reward rate: {}%",
                referral_rate as f64 / 100.0
            );
        }

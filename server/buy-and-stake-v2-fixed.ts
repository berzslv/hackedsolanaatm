/**
 * Buy And Stake - Version 2 (Fixed)
 * This is a completely reimplemented buy-and-stake flow that 
 * properly uses the staking vault contract
 */
...
import * as stakingVault from './staking-vault-client';
 ...
 * that correctly integrates with the staking vault program
 * 
...
export async function createCombinedBuyAndStakeTransactionV2(
  ...
  try {
    console.log(`Creating combined buy and stake transaction (v2) for ${userWalletAddress}, amount: ${amount}`);
    ...
    // 5. Add staking instruction using the proper staking vault program
    console.log(`Creating staking instruction for ${amount} tokens`);

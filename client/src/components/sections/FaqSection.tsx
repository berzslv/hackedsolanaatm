  ...
  return (
    <section id="faq" className="section section-odd">
      ...
      <div className="pattern-circles">
        <div className="absolute w-60 h-60 rounded-full border border-secondary/10 -top-20 -left-20"></div>
        <div className="absolute w-36 h-36 rounded-full border border-primary/10 -bottom-10 -right-10"></div>
        <div className="absolute w-5 h-5 bg-secondary/5 rounded-full top-1/3 right-1/4"></div>
        <div className="absolute w-8 h-8 bg-primary/5 rounded-full bottom-1/3 left-1/3"></div>
        <div className="absolute w-3 h-3 bg-accent/5 rounded-full top-2/3 right-1/3"></div>
      </div>
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`item-${index}`}
                className="bg-card/90 backdrop-blur-sm rounded-xl overflow-hidden border border-border"
              >
